let index = {
    init: function () {
        $("#save-button").on("click", () => { // function(){} , ()=>{} this를 바인딩하기 위해서!!
            this.save();
        });
        $("#login-button").on("click", () => { // function(){} , ()=>{} this를 바인딩하기 위해서!!
            this.login();
        });
    },
    ...
    save: function () {
        ...
        $.ajax({
            type: "POST", url: "/api/user/signup", data: JSON.stringify(data), // http body데이터
    ...
    login: function () {
        ...
        // ajax가 통신을 성공하고 서버가 json을 리턴해주면 자동으로 자바 오브젝트로 변환해주네요.
        $.ajax({
            type: "POST", url: "/api/user/login", data: JSON.stringify(data), // http body데이터
            contentType: "application/json; charset=utf-8",// body데이터가 어떤 타입인지(MIME)
